# Migrating Expo UI to Latent Studio Design System

Read `artifacts/latent-studio-ds/docs/AGENTS.md` and
`artifacts/latent-studio-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/latent-studio-ds/lib/native-theme` and
  `@workspace/latent-studio-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/latent-studio-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/latent-studio-ds` and retains only
