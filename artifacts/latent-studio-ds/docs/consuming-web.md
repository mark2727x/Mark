# Consuming Latent Studio Design System in web apps

Read `artifacts/latent-studio-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/latent-studio-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/latent-studio-ds/styles.css";
```
...
`node_modules/@workspace/latent-studio-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/latent-studio-ds/components/ui/button";
import { cn } from "@workspace/latent-studio-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/latent-studio-ds/hooks/use-toast";
```
...
`@workspace/latent-studio-ds/components/ui/button`. Run the app's typecheck
