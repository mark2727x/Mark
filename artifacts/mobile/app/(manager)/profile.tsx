...
function StarRating({ value }: { value: number }) {
  return (
    <View style={{ flexDirection: 'row', gap: 2 }}>
      {[1, 2, 3, 4, 5].map(i => (
        <Body key={i} style={{ color: i <= Math.round(value) ? c.primary : c.border, fontSize: 16 }}>★</Body>
      ))}
    </View>
  );
}
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.avatarSection}>
          ...
          <View style={{ gap: 4 }}>
            <H3>{user.name}</H3>
            <Caption style={styles.role}>Pool Manager</Caption>
            {user.ratingCount > 0 && (
        ...
        <Card style={styles.card}>
          <CardContent>
            <Label style={styles.cardLabel}>Payment info</Label>
            <Body style={styles.cardValue}>
              After a shift is completed, send payment via Zelle to the lifeguard's registered Zelle ID.
              The platform fee is split equally between both parties.
            </Body>
          </CardContent>
        </Card>
...
const styles = StyleSheet.create({
  ...
  avatarText: { color: c.primaryForeground, fontFamily: f.sansBold, fontSize: 28 },
